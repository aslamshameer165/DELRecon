% Zoom in by 100 units
if 2*(uu+100)<max(bbb(linedist<20)) % Check if the zoom in is within the range of intensity
   % Approve zoom in by 100 units and update the plots
    uu=uu+100; 
    hold on
   delete(S.sc)
   delete(S.pt)
